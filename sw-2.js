const $VERSION = "6935eb78a6a4a6e10c5d0264b99bcb86";
const $DEBUG = false;
const $Cache = {
    offline: true,
...
function applyEventStrategy(strategy, event) {
    ...
    switch (strategy.type) {
        case "offline-only":
            return fetchAndCache(request, strategy)().catch(
                getFromCache(request)
            );
        case "fallback-only":
            return fetchAndCache(request, strategy)().then(
                fallbackToCache(request)
            );
        case "prefer-cache":
            return getFromCache(request)().catch(
        ...
        .then((cache) => {
            return Promise.all(
                $Cache.precache.map((urlToPrefetch) => {
                    ...
                    const request = new Request(cacheBustedUrl, {
                        mode: "no-cors",
                    });
                    return fetch(request).then((response) => {
                        if (!isValidResponse(response)) {
                            logger.error(
                                `Failed for ${urlToPrefetch}.`,
                                "precaching"
                            );
                            return undefined;
                        }
